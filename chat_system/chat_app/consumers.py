...
def chat_receive(message):
    ...
    m = Message(user=message.user, message=data['message'], message_html=current_message)
    m.save()
...
@channel_session_user_from_http
def chathistory_connect(message):
    message.reply_channel.send({"accept": True})

@channel_session_user
def chathistory_receive(message):
    data = json.loads(message['text'])
    chat_queryset = Message.objects.filter(id__lte=data['last_message_id']).order_by("-created")[:10]
...
@channel_session_user
def chathistory_disconnect(message):
    pass